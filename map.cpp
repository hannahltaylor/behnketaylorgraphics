...
Map::Map(float **heightmap, int x, int y)
{
    this->x = x;
...
GLvoid* Map::diamondSquare(int start, int mapsize) {
	setElement(0, 0, start);
	setElement(0, mapsize, start);
	setElement(mapsize, 0, start);
	setElement(mapsize, mapsize, start);
	generate(mapsize + 1, 60, 0.59);
	capData(0, 255);
	smooth(1, 2);
    for (int i = 0; i < mapsize; i++) {
        for (int j = 0; j < mapsize; j++) {
            float noise = elements[i][j];
            heightmap[i][j] = noise;
        }
    }
